...
class TestTopLevelUrl:
    ...
    def test_top_level_url_beats_assets(self):
        creative = _make_creative(
            ...
            assets=build_assets(image_spec("main", url="https://asset.com/ad.png")),
        )
...
class TestPriorityKeys:
    ...
    def test_image_key(self):
        creative = _make_creative(assets=build_assets(image_spec("image", url="https://example.com/image.png")))
        assert _extract_url_from_assets(creative) == "https://example.com/image.png"

    def test_video_key(self):
        creative = _make_creative(assets=build_assets(image_spec("video", url="https://example.com/video.mp4")))
        assert _extract_url_from_assets(creative) == "https://example.com/video.mp4"

    def test_main_beats_image(self):
        creative = _make_creative(
            assets=build_assets(
                image_spec("image", url="https://example.com/image.png"),
                image_spec("main", url="https://example.com/main.png"),
            ),
        )
        assert _extract_url_from_assets(creative) == "https://example.com/main.png"
...
class TestFallback:
    ...
    def test_unknown_key_fallback(self):
        creative = _make_creative(
            assets=build_assets(image_spec("custom_banner", url="https://example.com/banner.png"))
        )
